...
class PointPain extends Component {
  ...
  checkAuthority = (id, authority) => {
    if (id) {
      const { roles = [] } =
        this.props.stateLoginValueAim?.user?.houses?.find(
          (e) => e.value === id
        ) || {};
  ...
  checkdone = (item) => {
    ...
    if (
      ...
      (moment(today).isAfter(data_d) ||
        (moment(today).isSame(data_d) && data_end <= ttime))
    ) {
      return true;
    }
    return false;
        ...
        <Grid container direction="row" alignItems="center">
          <Grid className="" item xs={12} sm={6} md={6}>
            <Grid container direction="row" alignItems="center">
              ...
              <Grid
                className="revwFiles addCssForTherapy"
                item
                xs={12}
                sm={4}
                md={4}
              ...
              {data.appointment_type ? (
                <>
                  <Grid className="attchNotePart">
                    <Grid
                      className={
                        data.status === "done" ? "attchDone" : "attchOpen"
                      }
                    >
                      <Button>
                        <label></label>
