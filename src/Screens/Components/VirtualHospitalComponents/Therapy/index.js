...
class Index extends Component {
  ...
  FinalServiceSubmit = () => {
    ...
    let { plz_enter_time, plz_enter_date } = translate;

    if (
      !this.state.selectedHouse ||
      (this.state.selectedHouse && !this.state.selectedHouse.value)
    ) {
                        ...
                        this.state.therapy_sequence.map((item, index) => (
                          <>
                            ...
                            {item?.type === "task" ? (
                              ...
                              <div>
                                <label>Assign Title : {item.title}</label>
                                <label>
                                  ...
                                  {item.services?.length > 0 &&
                                    item.services.map((cont) => (
                                      <>
                                        ...
                                        <span>{cont?.service_name}</span>
                                        <label>Service Amount</label>
                                        <span>{cont?.amount}</span>
                                        <label>Quantity</label>
                                        <span>{cont?.quantity}</span>
                                      </>
                                    ))}
                                </div>
                              </div>
