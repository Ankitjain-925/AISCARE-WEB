...
class Index extends Component {
  ...
  FinalServiceSubmit = () => {
    ...
    let { Something_went_wrong, Plz_select_a_Patient, Plz_select_a_staff } =
      translate;
    this.setState({ loaderImage: true });
    ...
    this.setState({ loaderImage: true });
    if (!data.patient || (data && data.patient && data.patient.length < 1)) {
  ...
  render() {
    ...
    return (
      <Grid className="newServc newServicAllSec">
        <Button onClick={() => this.handleOpenAss()}>{assignService}</Button>
        <Modal
            ...
            <Grid className="addSpeclContntIner2">
              ...
              <Grid className="enterServMain">
                <Grid className="enterSpcl">
                  ...
                  <Grid className="serviceAmountSec">
                    <p>{ServiceAmount}</p>
