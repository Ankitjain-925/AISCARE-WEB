...
class Index extends Component {
  ...
  DeleteUser = (deletekey, profile_id, bucket) => {
    ...
    axios.delete(sitedata.data.path + '/admin/deleteUser/' + UserId, commonHeader(user_token))
      .then((response) => {
        if (response &&
          response.data &&
          response.data.messages === "video chat account exists, deactivate that first"
        ) {
          confirmAlert({
            customUI: ({ onClose }) => {
              return (
                ...
                >
                  <p className="deletewarning">You are registered as a video conference account, for delete an account from Aimedis first you need to deactivate that account, After that you can delete this Aimedis account !</p>
                  <div className="react-confirm-alert-button-group react-confirm-alert-button-group1">
                    <button
                      onClick={() => {
                        onClose();
                      }}
